...
import { useGetOrderByIdQuery, usePayOrderMutation, useGetPayPalClientIdQuery, useDeliverOrderMutation } from "../Redux/slices/orderApiSlice";
...
const OrderScreen = () => {
  ...
  const [deliver, {isLoading: loadingDeliver}] = useDeliverOrderMutation();
        ...
        <Col md={8}>
          <ListGroup variant="flush">
            <ListGroup.Item style={{ backgroundColor: "inherit", color: "inherit" }}>
              ...
              {order.isDelivered ? (
                <Message variant={"success"}> {`Order Delivered at ${new Date(order.deliveredAt).toLocaleString()}`}</Message>
              ) : (
                ...
                <ListGroup.Item>
                  ...
                  {isPending ? (
                    ...
                    <div>
                      ...
                      <div>
                        <PayPalButtons style={{height: '20px'}} containerWidth={'10px'} createOrder={createOrder} onApprove={onApprove} onError={onError}></PayPalButtons>
                      </div>
                    </div>
                  )}
                </ListGroup.Item>
